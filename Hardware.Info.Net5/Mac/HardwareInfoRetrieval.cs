...
namespace Hardware.Info.Net5.Mac
{
    internal class HardwareInfoRetrieval : HardwareInfoBase, IHardwareInfoRetrieval
    {
        ...
        private async Task<bool> StartProcessAsync(string fileName, string arguments, Action<string> output, Action<string> error, int millisecondsTimeout = 60 * 1000)
        {
            using Process process = new Process();
    ...
    Hardware Overview:
      ...
      Hardware UUID: F6D9C340-725A-224A-8855-99AB8348F745
        ...
        public async Task<List<CPU>> GetCpuListAsync(bool includePercentProcessorTime = true)
        {
            ...
            string processOutput = await ReadProcessOutputAsync("sysctl", "-n machdep.cpu.brand_string");
            string[] info = processOutput.Split('@');
            ...
            processOutput = await ReadProcessOutputAsync("sysctl", "-n hw.l2cachesize");

            if (uint.TryParse(processOutput, out uint L2CacheSize))
                cpu.L2CacheSize = L2CacheSize;

            processOutput = await ReadProcessOutputAsync("sysctl", "-n hw.l3cachesize");

            if (uint.TryParse(processOutput, out uint L3CacheSize))
                cpu.L3CacheSize = L3CacheSize;

            processOutput = await ReadProcessOutputAsync("sysctl", "-n hw.physicalcpu");

            if (uint.TryParse(processOutput, out uint numberOfCores))
                cpu.NumberOfCores = numberOfCores;

            processOutput = await ReadProcessOutputAsync("sysctl", "-n hw.logicalcpu");
    ...
    Memory Slots:
      ...
      Upgradeable Memory: Yes
        ...
        public async Task<List<Memory>> GetMemoryListAsync()
        {
            ...
            await StartProcessAsync("system_profiler", "SPMemoryDataType",
                standardOutput =>
                {
                    string line = standardOutput.Trim();
...
Graphics/Displays:
    ...
    NVIDIA GeForce GT 650M:
      ...
      Displays:
        ...
        public async Task<List<Monitor>> GetMonitorListAsync()
        {
            List<Monitor> monitorList = new List<Monitor>();

            Monitor? monitor = null;

            await StartProcessAsync("system_profiler", "SPDisplaysDataType",
                standardOutput =>
                {
                    int spaceCount = standardOutput.TakeWhile(c => c == ' ').Count();
