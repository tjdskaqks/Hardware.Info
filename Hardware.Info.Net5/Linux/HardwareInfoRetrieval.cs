...
namespace Hardware.Info.Net5.Linux
{
    internal class HardwareInfoRetrieval : HardwareInfoBase, IHardwareInfoRetrieval
    {
        ...
        {
            string[] meminfo = await TryReadFileLinesAsync("/proc/meminfo");
        ...
        public async Task<List<Battery>> GetBatteryListAsync()
        {
            List<Battery> batteryList = new List<Battery>();

            string power_now = await TryReadFileTextAsync("/sys/class/power_supply/BAT0/power_now");
            string energy_full_design = await TryReadFileTextAsync("/sys/class/power_supply/BAT0/energy_full_design");
            string energy_full = await TryReadFileTextAsync("/sys/class/power_supply/BAT0/energy_full");
            string energy_now = await TryReadFileTextAsync("/sys/class/power_supply/BAT0/energy_now");
            ...
            Battery battery = new Battery
            {
                DesignCapacity = designCapacity,
                FullChargeCapacity = fullChargeCapacity,
                BatteryStatusDescription = await TryReadFileTextAsync("/sys/class/power_supply/BAT0/status"),
                EstimatedChargeRemaining = (ushort)(energyNow * 100 / fullChargeCapacity), // current charge remaining in percentage
                EstimatedRunTime = energyNow / powerNow, // current remaining life in minutes
                ExpectedLife = fullChargeCapacity / powerNow, // total expected lifetime in minutes
                MaxRechargeTime = fullChargeCapacity / powerNow, // total time, in minutes, to fully charge the battery
        ...
        public async Task<List<BIOS>> GetBiosListAsync()
        {
            List<BIOS> biosList = new List<BIOS>();

            BIOS bios = new BIOS
            {
                ReleaseDate = await TryReadFileTextAsync("/sys/class/dmi/id/bios_date"),
                Version = await TryReadFileTextAsync("/sys/class/dmi/id/bios_version"),
                Manufacturer = await TryReadFileTextAsync("/sys/class/dmi/id/bios_vendor")
            };
        ...
        public async Task<List<CPU>> GetCpuListAsync(bool includePercentProcessorTime = true)
        {
            List<CPU> cpuList = new List<CPU>();

            string[] lines = await TryReadFileLinesAsync("/proc/cpuinfo");
        ...
        private static void GetCpuUsage(ref CPU cpu)
        {
        ...
        private static async void GetCpuUsageAsync(CPU cpu)
        {
            // Column   Name    Description
            // 1        user    Time spent with normal processing in user mode.
            // 2        nice    Time spent with niced processes in user mode.
            ...
            string[] cpuUsageLineLast = await TryReadFileLinesAsync("/proc/stat");
            await Task.Delay(TimeSpan.FromMilliseconds(500));
            string[] cpuUsageLineNow = await TryReadFileLinesAsync("/proc/stat");
        ...
        public async Task<List<Drive>> GetDriveListAsync()
        {
            List<Drive> driveList = new List<Drive>();

            string processOutput = await ReadProcessOutputAsync("lshw", "-class disk");
        ...
        public async Task<List<Memory>> GetMemoryListAsync()
        {
            List<Memory> memoryList = new List<Memory>();

            string processOutput = await ReadProcessOutputAsync("lshw", "-short -C memory");
        ...
        public async Task<List<Motherboard>> GetMotherboardListAsync()
        {
            List<Motherboard> motherboardList = new List<Motherboard>();

            Motherboard motherboard = new Motherboard
            {
                Product = await TryReadFileTextAsync("/sys/class/dmi/id/board_name"),
                Manufacturer = await TryReadFileTextAsync("/sys/class/dmi/id/board_vendor"),
                SerialNumber = await TryReadFileTextAsync("/sys/class/dmi/id/board_serial")
            };
        ...
        public async Task<List<NetworkAdapter>> GetNetworkAdapterListAsync(bool includeBytesPersec = true, bool includeNetworkAdapterConfiguration = true)
        {
            List<NetworkAdapter> networkAdapterList = base.GetNetworkAdapterList(includeBytesPersec, includeNetworkAdapterConfiguration);
        ...
        public async Task<List<VideoController>> GetVideoControllerListAsync()
        {
            List<VideoController> videoControllerList = new List<VideoController>();

            string processOutput = await ReadProcessOutputAsync("lspci", string.Empty);
            ...
            foreach (string line in lines.Where(l => l.Contains("VGA compatible controller")))
            {
                ...
                if (split.Length > 2)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(relevant))
                    {
                        ...
                        videoControllerList.Add(new() { Description = relevant, Manufacturer = vendor, Name = name });
                    }
                }
            }
        ...
        public async Task<List<LinuxVolume>> GetDriveList2Async()
        {
            ...
            var outputList = await ReadProcessOutputAsync("df", "");
            string[] lines = outputList.Split(new[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
            if (lines.Length > 1)
            {
                foreach (var line in lines)
                {
                    if (line.Contains("filesystem", StringComparison.OrdinalIgnoreCase))
                        continue;

                    string[] infos = line.Split(" ", StringSplitOptions.RemoveEmptyEntries);
                    if (infos.Length > 0)
                    {
                        linuxVolumes.Add(new()
                        {
                            FileSystem = infos[0],
                            Size = Convert.ToUInt64(infos[1]),
                            UsedSpace = Convert.ToUInt64(infos[2]),
                            FreeSpace = Convert.ToUInt64(infos[3]),
                            UsePercent = infos[4],
                            Monunted = infos[5]
                        });
                    }
